module "virtual_site" {     
  ...
  f5xc_virtual_site_name                = format("%s-ipv6-all-sites", var.project_prefix)
  f5xc_virtual_site_type                = "CUSTOMER_EDGE"
  f5xc_virtual_site_selector_expression = [ format("site-mesh in (%s-ipv6-site)", var.project_prefix) ]
} 

resource "volterra_site_mesh_group" "smg" {
  name        = format("%s-ipv6-smg", var.project_prefix)
  namespace   = "system"
